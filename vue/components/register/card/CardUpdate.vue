...
onMounted(async () => {
  await cardStore.fetchCard();  // 초기 데이터 로드
  if (cardStore.currentCard) {
    currentCard.value = {...cardStore.currentCard};
  }
});

const saveOrUpdateCard = async() => {
  if (currentCard.value.id) {
    await cardStore.updateCard(currentCard.value);
  } else {
    await cardStore.createCard(currentCard.value);
  }
};
/*----------------------------양식 검증------------------------------------*/
import {nameRules, nullableRules} from "~/utils/form";
</script>

<template>
  <UiParentCard title="카드 조회 /수정">
