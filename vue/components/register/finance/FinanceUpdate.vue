<script setup>
  ...
  onMounted(async () => {
  await financeStore.fetchFinance();  // 초기 데이터 로드
  if (financeStore.currentFinance) {
  currentFinance.value = {...financeStore.currentFinance};
}
});

  const saveOrUpdateFinance = async () => {
  if (currentFinance.value.id) {
  await financeStore.updateFinance(currentFinance.value);
} else {
  await financeStore.createFinance(currentFinance.value);
}
};
/*----------------------------양식 검증------------------------------------*/
