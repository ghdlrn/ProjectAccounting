...
export const useCustomerStore = defineStore('customer', {
    ...
    actions: {
        async fetchCustomer(companyId: number) {
            try {
                const response = await useNuxtApp().$api.get(`/register/customer/company/${companyId}`);
        ...
        async createCustomer(companyId: number, data: Customer) {
            try {
                const response = await useNuxtApp().$api.post(`/register/customer/company/${companyId}`, data);
