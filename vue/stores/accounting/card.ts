...
export const useCardStore = defineStore('card', {
    ...
    actions: {
        async fetchCard() {
            try {
                const response = await apiClient().get(`/register/card`);
        ...
        async getCard(id: number) {
            try {
                const response = await apiClient().get(`/register/card/${id}`);
        ...
        async updateCard(data: Card) {
            try {
                const response = await apiClient().put(`/register/card/${data.id}`, data);
        ...
        async createCard(data: Card) {
            try {
                const response = await apiClient().post(`/register/card`, data);
                this.card.push(response.data);
                alert('카드 정보가 등록되었습니다');
            } catch (error: any) {
                alert('카드 등록 실패')
        ...
        async deleteCard(id: number) {
            try {
                const response = await apiClient().delete(`/register/card/${id}`);
